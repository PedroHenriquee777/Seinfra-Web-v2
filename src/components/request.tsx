...
export function RequestCard({
  ...
  const getVariant = (state: string): VariantType => {
    if (state === "Pendente") return "newOS";
    if (state === "Em execução") return "OSInProgess";
    return "default";
  };
      ...
      <DialogTrigger asChild>
        <div className={cn("bg-background rounded-3xl", "flex w-component-w cursor-pointer")}>
          <div
            className={cn(
              "bg-gray-500",
              !isExcluded && currentStatus.color,
              isExcluded && "bg-red-500",
                ...
                <div className="absolute right-5 self-start">
                  <div
                    className={cn(
                      ...
                      !isExcluded && currentStatus.color,
                      isExcluded && "bg-red-500",
                      State === "Em execução" && "px-1 lg:px-2",
                      "text-background text-xs rounded-sm",
                    )}
                  >
                    {State}
